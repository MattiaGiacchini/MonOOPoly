...
public class PropertyEffect extends AbstractCardDecorator {
	...
	private PropertyEffect(final Builder builder) {
		...
		for(var value : builder.buildingsToModify.entrySet()) {
			mapOfProperty.put(value.getKey(), this.calculateTheNumberOfBuildings
			              (value.getValue(), getLastChangeApplyed(value), rnd));
		}
